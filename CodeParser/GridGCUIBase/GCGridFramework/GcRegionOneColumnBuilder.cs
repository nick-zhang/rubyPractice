...
{
    public class GcRegionOneColumnBuilder : IGcRegionDefinitionBuilder
    {
        public GcRegionOneColumnBuilder(int regionWidth)
        {
        ...
        public GcRegionDefinition Build(IGcDataSlot titleSlot, List<GcDataTuple> parameterList)
        {
            ...
            foreach (var gcParameterTuple in parameterList)
            {
                paraRegion.AddRow(new GcRowDefintion()
                                      .AddCell(new GcCellDefinition(gcParameterTuple.Data,
                                                                    new GcSize(1, m_RegionWidth),
...
/*+- OmniWorks Replacement History - hzproductsapi`GCFramework`GridGCUIBase`GCGridFramework:GcRegionOneColumnBuilder.cs;4 */
